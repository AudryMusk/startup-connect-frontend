  ...
  <div v-else class="max-w-6xl mx-auto space-y-4 md:space-y-6 px-4 md:px-0">
      ...
      <div class="px-4 md:px-8 pb-4 md:pb-8">
        ...
        >
          ...
          <div class="flex flex-wrap items-center gap-2 md:gap-3">
            ...
            <Button
              v-if="canConnect && !connectionStatus"
            ...
            <Badge v-if="connectionStatus === 'accepted'" color="green" size="sm"
              >✓ Connectés</Badge
            >
          </div>
        </div>
          ...
          <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
            <div>
              ...
              >
                ...
                <span
                  v-if="user?.role === 'admin' && startup.rccm"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
      ...
      <div class="lg:col-span-2 space-y-4 md:space-y-6">
        ...
        <!-- Pending Members (Visible to Founder Only) -->
        <Card
          v-if="isOwner && pendingMembers.length > 0"
          class="border-2 border-orange-200 bg-orange-50"
        >
            ...
            >
              ...
              <div class="flex gap-2">
                <Button
                  size="sm"
                  variant="success"
                  @click="handleValidateMember(member.memberId)"
                ...
                <Button
                  size="sm"
                  variant="danger"
                  @click="handleRejectMember(member.memberId)"
                  :disabled="processingMember"
                >
                  <Icon name="X" :size="16" />
                  Refuser
        ...
        <Card>
          ...
          <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            >
              <div
                class="w-12 h-12 bg-theme-light rounded-full flex items-center justify-center text-theme font-bold"
              >
                {{ member.displayName?.[0] || '?' }}
              </div>
              <div class="flex-1 min-w-0">
                ...
                <Badge
                  v-if="!member.isValidated && !isOwner"
                  color="yellow"
                  size="sm"
                  class="mt-1"
                >
              ...
              <Badge v-if="member.role === 'fondateur'" color="theme" size="sm"
                >Fondateur</Badge
              >
            </div>
          </div>
      ...
      <div class="space-y-6">
        ...
        <Card>
          ...
          <div class="space-y-3 text-sm">
            ...
            <div class="flex items-start gap-2">
              <Icon
                :name="startup.verified ? 'CheckCircle' : 'AlertCircle'"
                :size="16"
...
const isOwner = computed(
  () => user.value?.role === 'startuper' && user.value?.startupId === route.params.id,
)
...
const loadData = async () => {
  ...
  try {
    ...
    const allMembers = (startupData.members || []).map((m) => ({
      uid: m.user_id || m.id,
      id: m.user_id || m.id,
      memberId: m.id, // ID from startup_members table
      ...
      isValidated: m.is_validated !== false, // true si validé ou si le champ n'existe pas (ancien membre)
    }))
